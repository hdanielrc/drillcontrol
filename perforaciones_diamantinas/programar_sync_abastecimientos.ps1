# ====================================================================
# Script PowerShell para Programar Sincronización Diaria de Abastecimientos
...
Write-Host "PROGRAMACIÓN DE SINCRONIZACIÓN DIARIA DE ABASTECIMIENTOS" -ForegroundColor Cyan
Write-Host "=====================================================================" -ForegroundColor Cyan
Write-Host ""

# Ruta del directorio actual (donde está el script)
$projectPath = $PSScriptRoot
$scriptPath = Join-Path $projectPath "sync_abastecimientos_diario.bat"

# Verificar que existe el script .bat
if (-not (Test-Path $scriptPath)) {
    Write-Host "❌ ERROR: No se encontró el archivo sync_abastecimientos_diario.bat" -ForegroundColor Red
...
try {
    ...
    Write-Host "¿Deseas ejecutar una prueba inmediata? (S/N): " -NoNewline -ForegroundColor Cyan
    ...
    if ($testResponse -eq "S" -or $testResponse -eq "s") {
        ...
        Write-Host "✓ Tarea iniciada. Revisa el archivo de logs para ver el resultado." -ForegroundColor Green
        Write-Host "Logs: $logsPath" -ForegroundColor White
    }
